...
class TestBuilderRequirements:
    ...
    @patch('faster_than_light.builder.build_ftl_gate')
    @patch('builtins.open', new_callable=mock_open)
    def test_main_with_multiple_requirements_files(self, mock_file, mock_build_ftl_gate):
...
# Empty line above should be ignored"""
    ...
    def test_main_requirements_file_not_found(self, mock_file, mock_build_ftl_gate):
        ...
        assert result.exit_code != 0
        assert "FileNotFoundError" in str(result.exception) or isinstance(result.exception, FileNotFoundError)
    
    @patch('faster_than_light.builder.build_ftl_gate')
    @patch('builtins.open')
    def test_main_requirements_file_permission_error(self, mock_file, mock_build_ftl_gate):
        ...
        assert result.exit_code != 0
        assert "PermissionError" in str(result.exception) or isinstance(result.exception, PermissionError)
...
class TestBuilderInterpreterHandling:
    ...
    def test_main_custom_interpreter(self, mock_build_ftl_gate):
        ...
        mock_build_ftl_gate.return_value = ("/custom_interp/gate.pyz", "custom_interp_hash")
        
        result = self.runner.invoke(main, ['--interpreter', '/opt/python/bin/python3.9'])
    ...
    def test_main_interpreter_short_option(self, mock_build_ftl_gate):
        ...
        mock_build_ftl_gate.return_value = ("/short_interp/gate.pyz", "short_interp_hash")
    ...
    def test_main_empty_interpreter_uses_default(self, mock_build_ftl_gate):
        ...
        mock_build_ftl_gate.return_value = ("/empty_interp/gate.pyz", "empty_interp_hash")
...
class TestBuilderIntegration:
    ...
    def test_main_realistic_ansible_scenario(self, mock_build_ftl_gate):
        ...
        result = self.runner.invoke(main, [
            ...
            '--module', 'template', 
            '--module', 'service',
            '--module-dir', '/usr/share/ansible/plugins/modules',
            '--module-dir', '/opt/custom/modules',
...
class TestBuilderErrorHandling:
    ...
    def test_main_import_error_handling(self, mock_build_ftl_gate):
        ...
        assert result.exit_code != 0
        assert "ImportError" in str(result.exception) or isinstance(result.exception, ImportError)
...
class TestBuilderEntryPoint:
    ...
    @patch('faster_than_light.builder.main')
    @patch('faster_than_light.builder.sys.argv', [
...
class TestBuilderEdgeCases:
    ...
    def test_main_with_duplicate_modules(self, mock_build_ftl_gate):
        ...
        result = self.runner.invoke(main, [
            '--module', 'test_module',
            '--module', 'test_module',  # Duplicate
            '--ftl-module', 'ftl_test',
            '--ftl-module', 'ftl_test'  # Duplicate
        ])
    ...
    def test_main_with_empty_module_names(self, mock_build_ftl_gate):
        ...
        result = self.runner.invoke(main, [
            '--module', '',
            '--ftl-module', '',  # Note: ftl-module is not used in current implementation
            '--module-dir', ''
        ])
    ...
    def test_main_with_special_characters(self, mock_build_ftl_gate):
        ...
        result = self.runner.invoke(main, [
            ...
            '--module', 'module_with_underscores',
            '--module-dir', '/path with spaces/modules',
            '--module-dir', '/path/with/unicode/🚀',
            '--interpreter', '/opt/python-3.9/bin/python3'
        ])
    ...
    def test_main_return_value_handling(self, mock_build_ftl_gate):
        ...
        for return_value in test_cases:
            ...
            expected_output = return_value if isinstance(return_value, str) else str(return_value)
...
class TestBuilderClick:
    ...
    def test_main_multiple_flag_values_as_tuples(self, mock_build_ftl_gate):
        ...
        result = self.runner.invoke(main, [
            '-m', 'mod1', '-m', 'mod2', '-m', 'mod3',
            '-f', 'ftl1', '-f', 'ftl2',
            '-M', 'dir1', '-M', 'dir2', '-M', 'dir3'
        ])
