...
class TestConnectStdinStdout:
    ...
    async def test_successful_pipe_connection(self, mock_stream_reader_cls, mock_protocol_cls, mock_stream_writer_cls, mock_get_loop):
...
class TestGetPythonPath:
    ...
    @patch('sys.path', ['/usr/lib/python3.9', '/usr/lib/python3.9/site-packages', '/custom/path'])
    @patch('os.pathsep', ':')
...
class TestGateRunModule:
    ...
    async def test_gate_run_module_from_bundle(self, mock_resources, mock_get_path, 
                                               mock_send, mock_check_output, mock_open_file,
                                               mock_env_copy, mock_rmtree, mock_mkdtemp):
        ...
        mock_files.joinpath.return_value.read_bytes.return_value = b'#!/usr/bin/python3\nprint("test")'
        mock_resources.return_value = mock_files
        
        mock_writer = MagicMock()
        
        await ftl_main.gate_run_module(mock_writer, "test_module", None, {"arg1": "value1"})
        
        # Verify module execution
        mock_mkdtemp.assert_called_once_with(prefix="ftl-module")
        mock_send.assert_called_once_with(
            mock_writer, "ModuleResult", 
    ...
    async def test_gate_run_module_from_message(self, mock_b64decode, mock_get_path,
                                                mock_send, mock_check_output, mock_open_file,
                                                mock_env_copy, mock_rmtree, mock_mkdtemp):
        ...
        await ftl_main.gate_run_module(mock_writer, "test_module", encoded_module, {"arg1": "value1"})
    ...
    @patch('importlib.resources.files')
    @patch('faster_than_light.ftl_gate.__main__.send_message')
    async def test_gate_run_module_not_found(self, mock_send, mock_resources, mock_open_file, mock_rmtree, mock_mkdtemp):
        """Test handling module not found error."""
        mock_mkdtemp.return_value = "/tmp/ftl-test"
        
        # Mock resources to raise FileNotFoundError
        mock_files = MagicMock()
        mock_files.joinpath.return_value.read_bytes.side_effect = FileNotFoundError("Module not found")
        mock_resources.return_value = mock_files
...
class TestMain:
    ...
    async def test_main_hello_message(self, mock_logging, mock_send, mock_read, mock_connect):
    ...
    @patch('logging.basicConfig')
    async def test_main_module_message(self, mock_logging, mock_gate_run, mock_send, mock_read, mock_connect):
    ...
    @patch('logging.basicConfig')
    async def test_main_ftl_module_message(self, mock_logging, mock_ftl_run, mock_send, mock_read, mock_connect):
    ...
    @patch('faster_than_light.ftl_gate.__main__.send_message')
    @patch('logging.basicConfig')
    async def test_main_shutdown_message(self, mock_logging, mock_send, mock_read, mock_connect):
    ...
    @patch('faster_than_light.ftl_gate.__main__.send_message')
    @patch('logging.basicConfig')
    async def test_main_unknown_message(self, mock_logging, mock_send, mock_read, mock_connect):
        ...
        assert result is None
        mock_send.assert_has_calls([
            call(mock_writer, "Error", {"message": "Unknown message type UnknownType"}),
            call(mock_writer, "Goodbye", {})
        ])
    ...
    @patch('faster_than_light.ftl_gate.__main__.send_message')
    @patch('logging.basicConfig')
    async def test_main_module_not_found_exception(self, mock_logging, mock_send, mock_read, mock_connect):
        ...
        with patch('faster_than_light.ftl_gate.__main__.gate_run_module') as mock_gate_run:
            ...
            assert result is None
            mock_send.assert_has_calls([
                call(mock_writer, "ModuleNotFound", 
    ...
    @patch('faster_than_light.ftl_gate.__main__.send_message')
    @patch('logging.basicConfig')
    async def test_main_system_error(self, mock_logging, mock_send, mock_read, mock_connect):
    ...
    @patch('faster_than_light.ftl_gate.__main__.send_message')
    @patch('logging.basicConfig')
    async def test_main_invalid_module_data(self, mock_logging, mock_send, mock_read, mock_connect):
    ...
    @patch('faster_than_light.ftl_gate.__main__.send_message')
    @patch('logging.basicConfig')
    async def test_main_invalid_ftl_module_data(self, mock_logging, mock_send, mock_read, mock_connect):
