...
class TestCleanUpFtlCache:
    ...
    def test_clean_up_existing_cache(
        self, mock_abspath, mock_expanduser, mock_isdir, mock_exists, mock_rmtree
    ):
    ...
    @patch("os.path.expanduser")
    @patch("os.path.abspath")
    def test_clean_up_nonexistent_cache(
        self, mock_abspath, mock_expanduser, mock_exists, mock_rmtree
    ):
    ...
    @patch("os.path.expanduser")
    @patch("os.path.abspath")
    def test_clean_up_cache_is_file(
        self, mock_abspath, mock_expanduser, mock_isdir, mock_exists, mock_rmtree
    ):
...
class TestCleanUpTmp:
    ...
    @patch("os.path.isdir")
    @patch("glob.glob")
    def test_clean_up_multiple_tmp_dirs(
        self, mock_glob, mock_isdir, mock_exists, mock_rmtree
    ):
    ...
    @patch("os.path.isdir")
    @patch("glob.glob")
    def test_clean_up_tmp_safety_check(
        self, mock_glob, mock_isdir, mock_exists, mock_rmtree
    ):
...
class TestProcessModuleResult:
    ...
    def test_process_gate_system_error(self):
        ...
        expected = {
            "error": {"error_type": "GateSystemError", "message": "Connection failed"}
        }
...
class TestUniqueHosts:
    ...
    def test_unique_hosts_multiple_groups(self):
        ...
        inventory = {
            "webservers": {"hosts": {"web1": {"ansible_host": "192.168.1.10"}}},
    ...
    def test_unique_hosts_duplicate_hosts(self):
        ...
        inventory = {
            "group1": {"hosts": {"server1": {"ansible_host": "192.168.1.10"}}},
            "group2": {
                "hosts": {"server1": {"ansible_host": "192.168.1.11"}}  # Different IP
            },
        }
        ...
        expected = {"server1": {"ansible_host": "192.168.1.11"}}
        assert result == expected
