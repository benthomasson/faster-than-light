...
class TestLoadInventory:
    ...
    def test_load_inventory_unicode_content(self, mock_yaml_load, mock_file):
        ...
        unicode_inventory = {
            "all": {
                "hosts": {
                    "服务器1": {"ansible_host": "192.168.1.10", "description": "测试服务器"},
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('yaml.safe_load')
    def test_load_inventory_returns_dict_for_falsy_yaml(self, mock_yaml_load, mock_file):
...
class TestLoadLocalhost:
    ...
    def test_load_localhost_default_interpreter(self):
        ...
        assert result["all"]["hosts"]["localhost"]["ansible_python_interpreter"] == sys.executable
    ...
    def test_load_localhost_custom_interpreter(self):
        ...
        assert result == expected
        assert result["all"]["hosts"]["localhost"]["ansible_python_interpreter"] == custom_interpreter
...
class TestWriteLocalhost:
    ...
    def test_write_localhost_default_filename(self, mock_load_localhost, mock_yaml_dump, mock_file):
        ...
        mock_yaml_dump.assert_called_once_with(test_inventory, stream=mock_file.return_value)
    
    @patch('builtins.open', new_callable=mock_open)
    @patch('yaml.dump')
    @patch('faster_than_light.inventory.load_localhost')
    def test_write_localhost_custom_filename(self, mock_load_localhost, mock_yaml_dump, mock_file):
    ...
    @patch('yaml.dump')
    @patch('faster_than_light.inventory.load_localhost')
    def test_write_localhost_yaml_dump_error(self, mock_load_localhost, mock_yaml_dump, mock_file):
    ...
    @patch('yaml.dump')
    @patch('faster_than_light.inventory.load_localhost')
    def test_write_localhost_complex_inventory(self, mock_load_localhost, mock_yaml_dump, mock_file):
        ...
        mock_yaml_dump.assert_called_once_with(complex_inventory, stream=mock_file.return_value)
...
class TestInventoryIntegration:
    ...
    def test_load_localhost_write_localhost_roundtrip(self):
        ...
        try:
            ...
            assert loaded_inventory["all"]["hosts"]["localhost"]["ansible_connection"] == "local"
    ...
    def test_custom_interpreter_roundtrip(self):
        ...
        try:
            ...
            with patch('faster_than_light.inventory.load_localhost', return_value=original_inventory):
                write_localhost(tmp_filename)
            ...
            assert loaded_inventory["all"]["hosts"]["localhost"]["ansible_python_interpreter"] == custom_interpreter
...
class TestInventoryEdgeCases:
    ...
    def test_load_inventory_large_file(self, mock_yaml_load, mock_file):
        ...
        large_inventory = {
            "all": {
                "hosts": {f"host_{i}": {"ansible_host": f"192.168.1.{i}"} for i in range(1000)}
            }
        }
    ...
    def test_load_inventory_deeply_nested(self, mock_yaml_load, mock_file):
        ...
        assert result["all"]["children"]["level1"]["children"]["level2"]["children"]["level3"]["hosts"]["deep_host"]["var"] == "value"
    ...
    def test_load_localhost_with_special_characters(self):
        ...
        assert result["all"]["hosts"]["localhost"]["ansible_python_interpreter"] == special_interpreter
    
    @patch('builtins.open', new_callable=mock_open)
    @patch('yaml.dump')
    @patch('faster_than_light.inventory.load_localhost')  
    def test_write_localhost_with_special_filename(self, mock_load_localhost, mock_yaml_dump, mock_file):
...
class TestInventoryErrorRecovery:
    ...
    @patch('yaml.dump')
    @patch('faster_than_light.inventory.load_localhost')
    def test_write_localhost_io_error_during_write(self, mock_load_localhost, mock_yaml_dump, mock_file):
