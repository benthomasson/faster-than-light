...
class TestRunModuleLocally:
    ...
    @patch("builtins.open", new_callable=mock_open)
    async def test_run_module_locally_binary_module(
        self, mock_file, mock_check_output, mock_is_binary, mock_copy, mock_mkdtemp
    ):
    ...
    @patch("faster_than_light.local.check_output")
    async def test_run_module_locally_new_style_module(
        self,
        mock_check_output,
        mock_is_new_style,
        mock_is_binary,
        ...
        mock_check_output.assert_called_once_with(
            "/usr/bin/python3 /tmp/test456/module.py", stdin=expected_stdin
        )
    ...
    @patch("faster_than_light.local.check_output")
    @patch("builtins.open", new_callable=mock_open)
    async def test_run_module_locally_want_json_module(
        self,
        mock_file,
        mock_check_output,
        mock_is_want_json,
        ...
        mock_check_output.return_value = (
            b'{"changed": true, "result": "json processed"}'
        )
    ...
    @patch("faster_than_light.local.check_output")
    @patch("builtins.open", new_callable=mock_open)
    async def test_run_module_locally_old_style_module(
        self,
        mock_file,
        mock_check_output,
        mock_is_want_json,
    ...
    @patch("faster_than_light.local.check_output")
    @patch("builtins.open", new_callable=mock_open)
    async def test_run_module_locally_old_style_none_args(
        self,
        mock_file,
        mock_check_output,
        mock_is_want_json,
    ...
    @patch("faster_than_light.local.check_output")
    @patch("builtins.open", new_callable=mock_open)
    async def test_run_module_locally_default_interpreter(
        self, mock_file, mock_check_output, mock_is_binary, mock_copy, mock_mkdtemp
    ):
    ...
    @patch("faster_than_light.local.check_output")
    @patch("builtins.open", new_callable=mock_open)
    async def test_run_module_locally_invalid_json_output(
        self, mock_file, mock_check_output, mock_is_binary, mock_copy, mock_mkdtemp
    ):
    ...
    @patch("builtins.open", new_callable=mock_open)
    async def test_run_module_locally_exception_handling(
        self,
        mock_file,
        mock_print_exc,
        mock_check_output,
...
class TestRunFtlModuleLocally:
    ...
    async def test_run_ftl_module_locally_complex_args(self, mock_run_path):
        ...
        result = await run_ftl_module_locally(
            "host", {}, "/path/to/module.py", complex_args
        )
    ...
    async def test_run_ftl_module_locally_main_execution_error(self, mock_run_path):
        ...
        with pytest.raises(ValueError, match="Invalid parameter"):
            await run_ftl_module_locally(
                "host", {}, "/path/to/failing.py", {"bad_param": "value"}
            )
...
class TestLocalIntegration:
    ...
    @patch("faster_than_light.local.check_output")
    @patch("builtins.open", new_callable=mock_open)
    async def test_module_type_detection_workflow(
        self,
        mock_file,
        mock_check_output,
        mock_is_want_json,
...
class TestLocalEdgeCases:
    ...
    async def test_run_ftl_module_locally_unicode_args(self, mock_run_path):
        ...
        result = await run_ftl_module_locally(
            "host", {}, "/path/to/module.py", unicode_args
        )

        assert result == ("host", {"unicode": "processed"})
        mock_main.assert_called_once_with(
            message="Hello 世界", emoji="🚀🌟", path="/路径/文件.txt"
        )
