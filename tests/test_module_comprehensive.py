...
class TestRunModuleCore:
    ...
    async def test_run_module_basic_execution(
        self,
        mock_gather,
        mock_create_task,
        mock_build_gate,
    ...
    @patch("asyncio.create_task")
    @patch("asyncio.gather", new_callable=AsyncMock)
    async def test_run_module_with_refs(
        self, mock_gather, mock_create_task, mock_unique_hosts, mock_find_module
    ):
        """Test module execution with Ref objects for dynamic variables."""
        mock_find_module.return_value = "/path/to/module.py"
        mock_unique_hosts.return_value = {"host1": {"name": "WebServer1", "port": 80}}
    ...
    @patch("asyncio.create_task")
    @patch("asyncio.gather", new_callable=AsyncMock)
    async def test_run_module_with_host_args(
        self, mock_gather, mock_create_task, mock_unique_hosts, mock_find_module
    ):
    ...
    @patch("asyncio.create_task")
    @patch("asyncio.gather", new_callable=AsyncMock)
    async def test_run_module_chunking_behavior(
        self,
        mock_gather,
        mock_create_task,
        mock_chunk,
    ...
    @patch("faster_than_light.module.find_module")
    @patch("faster_than_light.module.unique_hosts")
    async def test_run_module_with_custom_gate_builder(
        self, mock_unique_hosts, mock_find_module
    ):
...
class TestRunModuleSync:
    ...
    def test_run_module_sync_gate_cache_warning(
        self, mock_print, mock_new_loop, mock_run_module
    ):
...
class TestIntegrationScenarios:
    ...
    @patch("faster_than_light.module.run_module_remotely")
    @patch("asyncio.gather", new_callable=AsyncMock)
    async def test_mixed_local_remote_execution(
        self, mock_gather, mock_run_remotely, mock_unique_hosts, mock_find_module
    ):
    ...
    @patch("asyncio.create_task")
    @patch("asyncio.gather", new_callable=AsyncMock)
    async def test_complex_ref_and_host_args_combination(
        self,
        mock_gather,
        mock_create_task,
        mock_deref,
...
class TestErrorHandling:
    ...
    @patch("faster_than_light.module._run_module")
    @patch("asyncio.new_event_loop")
    def test_run_module_sync_exception_propagation(
        self, mock_new_loop, mock_run_module
    ):
