...
class TestBuildFtlGateBasic:
    ...
    def test_build_ftl_gate_minimal_parameters(self, mock_sha256, mock_exists, mock_ensure_dir):
    ...
    @patch('faster_than_light.gate.os.path.exists')
    @patch('faster_than_light.gate.hashlib.sha256')
    def test_build_ftl_gate_with_all_parameters(self, mock_sha256, mock_exists, mock_ensure_dir):
...
class TestBuildFtlGateCaching:
    ...
    def test_build_ftl_gate_cache_hit(self, mock_logger, mock_sha256, mock_exists, mock_ensure_dir):
    ...
    @patch('faster_than_light.gate.os.path.exists')
    @patch('faster_than_light.gate.hashlib.sha256')
    def test_build_ftl_gate_cache_miss_triggers_build(self, mock_sha256, mock_exists, mock_ensure_dir):
        ...
        with patch('faster_than_light.gate.tempfile.mkdtemp') as mock_mkdtemp, \
            ...
            mock_files.return_value.joinpath.return_value.read_text.return_value = "# gate main"
...
class TestBuildFtlGateModuleInstallation:
    ...
    @patch('faster_than_light.gate.shutil.rmtree')
    @patch('faster_than_light.gate.shutil.copy')
    def test_build_ftl_gate_installs_modules(self, mock_copy, mock_rmtree, mock_zipapp,
                                           mock_read_module, mock_find_module, mock_files,
                                           mock_makedirs, mock_mkdir, mock_mkdtemp, 
                                           mock_sha256, mock_exists, mock_ensure_dir):
        ...
        with patch('builtins.open', mock_open()) as mock_file:
            ...
            assert mock_file.call_count >= 4  # At least __main__.py, __init__.py, and 2 modules
    
    @patch('faster_than_light.gate.ensure_directory')
    @patch('faster_than_light.gate.os.path.exists')
    @patch('faster_than_light.gate.hashlib.sha256')
    @patch('faster_than_light.gate.find_module')
    def test_build_ftl_gate_module_not_found_error(self, mock_find_module, mock_sha256, 
                                                  mock_exists, mock_ensure_dir):
        ...
        with patch('faster_than_light.gate.tempfile.mkdtemp'), \
             patch('faster_than_light.gate.os.mkdir'), \
             patch('faster_than_light.gate.os.makedirs'), \
             patch('builtins.open', mock_open()), \
             patch('faster_than_light.gate.files'):
...
class TestBuildFtlGateDependencies:
    ...
    @patch('faster_than_light.gate.shutil.rmtree')
    @patch('faster_than_light.gate.shutil.copy')
    def test_build_ftl_gate_installs_dependencies(self, mock_copy, mock_rmtree, mock_zipapp,
                                                 mock_check_output, mock_files, mock_makedirs,
                                                 mock_mkdir, mock_mkdtemp, mock_sha256,
                                                 mock_exists, mock_ensure_dir):
        ...
        with patch('builtins.open', mock_open()) as mock_file:
            build_ftl_gate(dependencies=dependencies, local_interpreter=local_interpreter)
            
            # Verify requirements.txt was written
            requirements_calls = [call for call in mock_file.call_args_list 
            ...
            expected_command = [
                local_interpreter,
                "-m", "pip", "install",
                "-r", "/tmp/dep_build/requirements.txt",
                "--target", "/tmp/dep_build/ftl_gate"
            ]
    ...
    @patch('faster_than_light.gate.files')
    @patch('faster_than_light.gate.check_output')
    def test_build_ftl_gate_pip_failure(self, mock_check_output, mock_files, mock_makedirs,
                                       mock_mkdir, mock_mkdtemp, mock_sha256, mock_exists,
                                       mock_ensure_dir):
        ...
        mock_sha256.return_value = mock_hash_obj
        mock_files.return_value.joinpath.return_value.read_text.return_value = "# main"
        mock_check_output.side_effect = CalledProcessError(1, "pip", b"Package not found")
...
class TestBuildFtlGateZipappCreation:
    ...
    @patch('faster_than_light.gate.shutil.rmtree')
    @patch('faster_than_light.gate.shutil.copy')
    def test_build_ftl_gate_creates_zipapp(self, mock_copy, mock_rmtree, mock_zipapp,
                                         mock_files, mock_makedirs, mock_mkdir,
                                         mock_mkdtemp, mock_sha256, mock_exists,
                                         mock_ensure_dir):
        ...
        with patch('builtins.open', mock_open()):
            ...
            mock_zipapp.assert_called_once_with(
                "/tmp/zip_build/ftl_gate",
                "/tmp/zip_build/ftl_gate.pyz",
                interpreter
            )
...
class TestBuildFtlGateFileOperations:
    ...
    @patch('faster_than_light.gate.shutil.rmtree')
    @patch('faster_than_light.gate.shutil.copy')
    def test_build_ftl_gate_directory_structure(self, mock_copy, mock_rmtree, mock_zipapp,
                                              mock_files, mock_makedirs, mock_mkdir,
                                              mock_mkdtemp, mock_sha256, mock_exists,
                                              mock_ensure_dir):
    ...
    @patch('faster_than_light.gate.os.makedirs')
    @patch('faster_than_light.gate.files')
    def test_build_ftl_gate_file_write_failure(self, mock_files, mock_makedirs, mock_mkdir,
                                              mock_mkdtemp, mock_sha256, mock_exists,
                                              mock_ensure_dir):
...
class TestBuildFtlGateIntegration:
    ...
    @patch('faster_than_light.gate.shutil.rmtree')
    @patch('faster_than_light.gate.shutil.copy')
    def test_build_ftl_gate_full_workflow(self, mock_copy, mock_rmtree, mock_zipapp,
                                        mock_check_output, mock_read_module, mock_find_module,
                                        mock_files, mock_makedirs, mock_mkdir, mock_mkdtemp,
                                        mock_sha256, mock_exists, mock_ensure_dir):
        ...
        mock_sha256.return_value = mock_hash_obj
        mock_files.return_value.joinpath.return_value.read_text.return_value = "#!/usr/bin/env python3\n# Gate main"
    ...
    def test_build_ftl_gate_debug_logging(self, mock_logger):
        ...
        with patch('faster_than_light.gate.ensure_directory'), \
            ...
            build_ftl_gate(modules=modules, module_dirs=module_dirs, dependencies=dependencies)
...
class TestBuildFtlGateEdgeCases:
    ...
    @patch('faster_than_light.gate.os.path.exists')
    @patch('faster_than_light.gate.hashlib.sha256')
    def test_build_ftl_gate_empty_lists(self, mock_sha256, mock_exists, mock_ensure_dir):
    ...
    @patch('faster_than_light.gate.os.path.exists')
    @patch('faster_than_light.gate.hashlib.sha256')
    def test_build_ftl_gate_special_characters_in_parameters(self, mock_sha256, mock_exists, mock_ensure_dir):
    ...
    @patch('faster_than_light.gate.hashlib.sha256')
    @patch('faster_than_light.gate.tempfile.mkdtemp')
    def test_build_ftl_gate_temp_directory_failure(self, mock_mkdtemp, mock_sha256, 
                                                  mock_exists, mock_ensure_dir):
