...
async def connect_stdin_stdout() -> Tuple[
    Union[asyncio.StreamReader, StdinReader],
    Union[asyncio.StreamWriter, StdoutWriter],
]:
    ...
    try:
        ...
        stream_writer = asyncio.StreamWriter(
            w_transport, w_protocol, stream_reader, loop
        )
...
def send_message(
    writer: Union[asyncio.StreamWriter, StdoutWriter], msg_type: str, data: Any
) -> None:
...
async def check_output(
    cmd: str, env: Optional[Dict[str, str]] = None, stdin: Optional[bytes] = None
) -> Tuple[bytes, bytes]:
...
async def gate_run_module(
    ...
    try:
        ...
        if is_binary_module(module_bytes):
            ...
            logger.info(
                f"ANSIBLE_MODULE_ARGS {json.dumps(dict(ANSIBLE_MODULE_ARGS=module_args))}"
            )
    ...
    while True:
        try:
            ...
            send_message(
                writer,
                "ModuleNotFound",
                dict(message=f"Module {e} not found in gate bundle."),
            )
